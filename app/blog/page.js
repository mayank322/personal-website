...
export default function Blog() {
  ...
  return (
    <div style={{ background: '#faf9f7', minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', maxWidth: '860px', margin: '0 auto', padding: '0 48px', position: 'relative' }}>

        {/* Main content */}
        <div style={{ flex: 1, transition: 'all 0.3s', paddingRight: activePost ? '24px' : '0' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px 20px' }}>
              <div style={{ marginBottom: '16px' }}>
                {(() => { const tc = tagColors[activePost.tag] || tagColors.Tech; return (
                  <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '20px', background: tc.bg, color: tc.color }}>{activePost.tag}</span>
                ); })()}
              </div>
              ...
              <div style={{ borderTop: '0.5px solid #e5e2d9', paddingTop: '20px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  {[['X', `https://x.com/intent/tweet?text=${encodeURIComponent(activePost.title)}&url=${encodeURIComponent(activePost.link || '')}`], ['LinkedIn', `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(activePost.link || '')}`]].map(([label, url]) => (
                  ...
                  <button onClick={() => navigator.clipboard.writeText(activePost.link || window.location.href)}
